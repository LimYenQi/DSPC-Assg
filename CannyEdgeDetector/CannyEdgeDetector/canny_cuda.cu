...
void canny_edge_detect(const uint8_t* input_image, int height, int width, int high_threshold, int low_threshold, uint8_t* output_image) {
    ...
    checkCudaErrors(cudaMalloc((void**)&gaussian_blur_input, image_size * sizeof(uint8_t)));
    checkCudaErrors(cudaMalloc((void**)&gaussian_blur_output, image_size * sizeof(uint8_t)));
    checkCudaErrors(cudaMemcpy(gaussian_blur_input, input_image, image_size * sizeof(uint8_t), cudaMemcpyHostToDevice));
    ...
    checkCudaErrors(cudaMalloc((void**)&gradient_magnitude, image_size * sizeof(double)));
    checkCudaErrors(cudaMalloc((void**)&gradient_direction, image_size * sizeof(uint8_t)));
    ...
    // Non-max Suppression
    double* nms_output;
    checkCudaErrors(cudaMalloc((void**)&nms_output, image_size * sizeof(double)));
    ...
    non_max_suppression << <n_blocks, n_threads >> > (gradient_magnitude, gradient_direction, height, width, nms_output);
    checkCudaErrors(cudaPeekAtLastError());
    checkCudaErrors(cudaDeviceSynchronize());
    // cout << "Launching non-max suppression kernel finished." << endl;
    double* nms_output_h = new double[image_size];
    checkCudaErrors(cudaMemcpy(nms_output_h, nms_output, image_size * sizeof(double), cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaFree(gradient_magnitude));
    checkCudaErrors(cudaFree(gradient_direction));

    // Thresholding
    uint8_t* double_thresh_output;
    checkCudaErrors(cudaMalloc((void**)&double_thresh_output, image_size * sizeof(uint8_t)));
    // cout << "Launching non-max suppression kernel..." << endl;
    thresholding << <n_blocks, n_threads >> > (nms_output, height, width, high_threshold, low_threshold, double_thresh_output);
    checkCudaErrors(cudaPeekAtLastError());
    checkCudaErrors(cudaDeviceSynchronize());
    // cout << "Launching non-max suppression kernel finished." << endl;
    uint8_t* double_thresh_output_h = new uint8_t[image_size];
    checkCudaErrors(cudaMemcpy(double_thresh_output_h, double_thresh_output, image_size * sizeof(uint8_t), cudaMemcpyDeviceToHost));
    ...
    printf("Time taken for CUDA: %lfs\n", delta / 1000);
}
...
__global__ void gradient_magnitude_direction(const uint8_t* input_image, int height, int width, double* magnitude, uint8_t* direction) {
    ...
    if (grad_x_sum == 0.0 || grad_y_sum == 0.0) {
        ...
        magnitude[pixel_index] = ((std::sqrt((grad_x_sum * grad_x_sum) + (grad_y_sum * grad_y_sum))));
