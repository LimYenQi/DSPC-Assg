...
void canny_edge_detect(const uint8_t* input_image, int height, int width,
    ...
    gradient_magnitude_direction(gaussian_blur_output, height, width, gradient_magnitude, gradient_direction);
    non_max_suppression(gradient_magnitude, gradient_direction, height, width, nms_output);
    thresholding(nms_output, height, width, high_threshold, low_threshold, double_thresh_output);
